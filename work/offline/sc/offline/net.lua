...
function Ctx:reply(tData, strFunc, strObj)
	OfflineNet:push(strObj or self.objName, strFunc or self:defaultHandler(), { tData })
end
...
function Ctx:call(strObj, strFunc, ...)
	OfflineNet:push(strObj, strFunc, { ... })
end

--[[ Tên handler client mặc định.
	Suy luận giống build_spec.candidates(): bỏ tiền tố Client rồi thêm On. ]]
function Ctx:defaultHandler()
	local base = self.funcName
	local stripped = string.match(base, "^Client(.+)$")
	return "On" .. (stripped or base)
end
...
function OfflineNet:flushOne(item)
	local okEnc, szData = pcall(cjson.encode, item.args)
	if not okEnc then
...
function OfflineNet:dispatch(nModule, strObj, strFunc, ...)
	...
	if not fn then
		if not OfflineRouter:isSilent(strFunc) then
			OfflineLog:missing(nModule, strObj, strFunc, { ... })
		end
		return
	end
...
function OfflineNet:install()
	...
	if CUIRPCManager then
		...
		CUIRPCManager.StartRPC = function(mgr, szIP, bNeedToResponse, szCallbackLuaObj,
			...
			OfflineNet:push(szCallbackLuaObj, szCallback, { false, true })
			return true
		end
