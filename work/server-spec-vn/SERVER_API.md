# Đặc tả API server — Búa Tạ (`com.cmn.buatanew 1.26.81485`)

Trích tự động từ 973 file Lua client đã giải mã. Mọi số liệu dưới đây đến từ chính mã nguồn, không suy đoán.

| | |
|---|---|
| Hàm server được client gọi | **614** (702 chỗ gọi) |
| — khai báo trực tiếp `CallServer("Tên", …)` | 487 |
| — sinh lúc chạy qua `bindRpcToEvent` ⟳ | 127 |
| Trong đó dò được handler trả về | 415 |
| Callback được API ở §3 dùng tới | 410 |
| Tổng hàm `On*` có trong client | 944 |
...
| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| **`TickFromClient`** | `()` | `—` | tick to game gateway server | `sc/user/UI/CUIMain.lua:1141` |

#### ClientGameGateway

| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| **`Handshake`** | `(strKey: string, tInfo: table)` | `—` |  | `sc/user/Logical/ClientGameGateway.lua:285` |

### 3.4 `SERVER.GAME_LOGIC` — 531 API
...
| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| `G_EpicChapterLogic`.**`ClientBuyEpicGoods`** | `(tonumber: any)` | `—` |  | `sc/user/UI/CUIExchangeShop.lua:190` |
| `G_GuildStoreLogic`.**`ClientBuyGuildWarGoods`** | `(tonumber: any)` | `—` |  | `sc/user/UI/CUIExchangeShop.lua:187` |
...
| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| `G_ChapterLogic`.**`ClientCheck`** | `(tData: table)` | `—` |  | `sc/user/Battle/CUIGame.lua:840` |
...
| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| **`ClientGMCommond`** | `({"channeldemouser"}: table)` | `OnGMCommond` | 创建昵称回调 | `sc/user/UI/CUILogin2.lua:2894` |
| `G_GameWorld`.**`ClientUpdateToken`** | `(tInfo: table)` | `—` | ]] | `sc/user/UI/CUILogin2.lua:596` |
...
| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| `G_RechargeLogic`.**`ClientGetUserBalance`** | `()` | `OnGetUserBalance` |  | `sc/user/UI/CUIRechargeStore.lua:231` |
| `G_RechargeLogic`.**`ClientViRechargeCompleteDealPayment`** | `(tVIData: table)` | `—` |  | `sc/user/UI/CUIRechargeStore.lua:2460` |
...
| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| `G_ActivityShoppingLogic`.**`ClientActivityShopping`** | `(TimeLimitDiscountSale: any, szGoods: string, dwBuyNum: number)` | `OnActivityShopping` | 购买 | `sc/user/Logical/ClientActivitiesLogic.lua:389` |
| `G_ActivityFund`.**`ClientBuyActivityFund`** | `(strFundType: string)` | `OnBuyActivityFund` | 购买活动基金 | `sc/user/Logical/ClientActivitiesLogic.lua:283` |
...
| `G_ActivityFund`.**`ClientGetLevelFundPrize`** | `()` | `—` | 获取等级基金奖励 | `sc/user/Logical/ClientActivitiesLogic.lua:297` |
| `G_MonthlyCardLogic`.**`ClientGetReward`** | `(sCardType: string)` | `OnGetReward` | 获得每日奖励 | `sc/user/Logical/ClientActivitiesLogic.lua:370` |
| `G_ActivityFund`.**`ClientGetTimeFundPrize`** | `()` | `—` | 获取时间基金奖励 | `sc/user/Logical/ClientActivitiesLogic.lua:312` |
| `G_ActivityShoppingLogic`.**`ClientQueryActivityShoppingLimit`** | `(TimeLimitDiscountSale: any)` | `OnQueryActivityShoppingLimit` | 查询 | `sc/user/Logical/ClientActivitiesLogic.lua:400` |
...
#### ClientJiuFaZhongYuan

| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| `G_JiuFaZhongyuan`.**`ClientBeginBattle`** ⟳ | `(tFightHeroIDList: table, nLevel: number)` | `OnBeginBattle` | 开始战斗 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientEndBattle`** ⟳ | `(tbHeroDamageHp: table)` | `OnEndBattle` | 开始战斗 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetAllCampStep`** ⟳ | `()` | `OnGetAllCampStep` | 所有副本进度排行榜 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetAllCompleteChapterRank`** ⟳ | `()` | `OnGetAllCompleteChapterRank` | 查询自己完成副本信息 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetCampDamageRank`** ⟳ | `(nLevel: number, 1: number = 1, 50: number = 50)` | `OnGetCampDamageRank` | 伤害排行 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetCampRankInfo`** ⟳ | `()` | `—` | 获取阵营排行榜 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetChapterStepInfo`** ⟳ | `(nLevel: number)` | `OnGetChapterStepInfo` | 获取关卡进度排行榜 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetCompleteAward`** ⟳ | `(nLevel: number)` | `OnGetCompleteAward` | 获取通关奖励 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetMVPInfo`** ⟳ | `()` | `OnGetMVPInfo` | mvp信息 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetPlayerRankInfo`** ⟳ | `()` | `—` | 获取单人的伤害排行榜 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientGetStepAward`** ⟳ | `(nLevel: number, nPercent: number)` | `OnGetStepAward` | 获取单关进度奖励 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientJoinCamp`** ⟳ | `()` | `OnJoinCamp` | 加入阵营 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientOneKeyAward`** ⟳ | `()` | `OnOneKeyAward` | 一键领奖 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientSetChallengedHeroIdList`** ⟳ | `()` | `—` | 上报挑战列表 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
| `G_JiuFaZhongyuan`.**`ClientUpdateHp`** ⟳ | `(g_CUIJFZYDeploy: any, tbBossDamageHp: table)` | `OnUpdateHp` | 血量上报 | `sc/user/UI/JFZY/ClientJiuFaZhongYuan.lua:11` |
...
| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| `G_RechargeLogic`.**`ClientCompleteAibeiPayOrder`** | `(orderId: any)` | `OnCompleteAibeiPayOrder` |  | `sc/user/Logical/ClientRechargeLogic.lua:335` |
| `G_RechargeLogic`.**`ClientGetAiBeiPayOrder`** | `(nProductId: number)` | `OnGetAiBeiPayOrder` |  | `sc/user/Logical/ClientRechargeLogic.lua:319` |
...
| `G_ServerStateWarProtocol`.**`ClientGetAllMovingPaths`** ⟳ | `(__nMapMovePath: any)` | `—` | 获取xx个玩家的移动路径 | `sc/user/UI/StateWar/StateWarLogic.lua:5` |
| `G_ServerStateWarProtocol`.**`ClientGetAttackCount`** ⟳ | `()` | `—` | 查看国战攻击次数，当天可购买次数 | `sc/user/UI/StateWar/StateWarLogic.lua:5` |
| `G_ServerStateWarProtocol`.**`ClientGetCountryNotice`** ⟳ | `()` | `—` | 获取公告 | `sc/user/UI/StateWar/StateWarLogic.lua:5` |
...
| `G_ServerStateWarProtocol`.**`ClientGetMovingHistory`** | `()` | `OnGetMovingHistory` | 获取国战移动路径 | `sc/user/UI/StateWar/StateWarLogic.lua:71` |
...
| `G_ServerStateWarProtocol`.**`ClientSetCountryNotice`** ⟳ | `(strNotice: string)` | `OnSetCountryNotice` | 设置公告 | `sc/user/UI/StateWar/StateWarLogic.lua:5` |
...
| `G_ServerStateWarProtocol`.**`ClientViewPath`** ⟳ | `()` | `—` | 查看移动路径 | `sc/user/UI/StateWar/StateWarLogic.lua:5` |

#### StockData

| Hàm server | Tham số | Handler trả về | Mô tả | Nguồn |
|---|---|---|---|---|
| `G_ActivityStock`.**`ClientBuyStock`** ⟳ | `(strStockId: string, nCount: number, nPrice: number)` | `OnBuyStock` | 买股票 | `sc/user/UI/Stock/StockData.lua:11` |
| `G_ActivityStock`.**`ClientBuyStockGoods`** ⟳ | `(tonumber: any)` | `OnBuyStockGoods` | 兑换商城道具 | `sc/user/UI/Stock/StockData.lua:11` |
| `G_ActivityStock`.**`ClientFreshStockDetailTrend`** ⟳ | `()` | `—` | 刷新股票详情走势图 | `sc/user/UI/Stock/StockData.lua:11` |
| `G_ActivityStock`.**`ClientGetShopConf`** ⟳ | `()` | `OnGetShopConf` | 获取商城道具配置 | `sc/user/UI/Stock/StockData.lua:11` |
| `G_ActivityStock`.**`ClientQueryAccount`** ⟳ | `()` | `OnQueryAccount` | 查询用户帐户 | `sc/user/UI/Stock/StockData.lua:11` |
| `G_ActivityStock`.**`ClientQueryStock`** ⟳ | `()` | `OnQueryStock` | 查询当前股票价格 | `sc/user/UI/Stock/StockData.lua:11` |
| `G_ActivityStock`.**`ClientQueryStockAllToday`** ⟳ | `()` | `OnQueryStockAllToday` | 查询今日股票价格 | `sc/user/UI/Stock/StockData.lua:11` |
| `G_ActivityStock`.**`ClientSellStock`** ⟳ | `(strStockId: string, nCount: number, nPrice: number)` | `OnSellStock` | 卖股票 | `sc/user/UI/Stock/StockData.lua:11` |
...
| **`OnActivityShopping`** | `ClientActivityShoppingLogic` | `dwSucBuyNum` | — | `sc/user/Logical/ClientActivitiesLogic.lua:393` |
...
| **`OnBeginBattle`** | `CUIJFZYDeploy` | `tData` | — | `sc/user/UI/JFZY/CUIJFZYDeploy.lua:1247` |
| **`OnBeginFightStateWar`** | `StateWarBattleData` | `data` | — | `sc/user/UI/StateWar/StateWarBattleData.lua:320` |
...
| **`OnBuyStock`** | `CUIStockDealDlg` | `data` | — | `sc/user/UI/Stock/CUIStockDealDlg.lua:212` |
| **`OnBuyStockGoods`** | `CUIExchangeShop` | `data` | `costPrizeData` | `sc/user/UI/CUIExchangeShop.lua:497` |
...
| **`OnCompeleteArenaFight`** | `CUIGame` | `tData` | `bResult` | `sc/user/Battle/CUIGame.lua:4540` |
...
| **`OnComplateFightStateWar`** | `StateWarBattleData` | `data` | — | `sc/user/UI/StateWar/StateWarBattleData.lua:338` |
| **`OnCompleteAibeiPayOrder`** | `RechargeLogic` | `nErrorCode` | — | `sc/user/Logical/ClientRechargeLogic.lua:338` |
...
| **`OnEndBattle`** | `FBJiuFaZhongYuan` | `data` | — | `sc/user/FB/FBJiuFaZhongYuan.lua:140` |
...
| **`OnEnterStateWarScene`** | `StateWarBattleData` | `data` | `hero` | `sc/user/UI/StateWar/StateWarBattleData.lua:73` |
...
| **`OnFirewood`** | `CUIArmyGroupCampsite` | `returnData` | `UserFireWoodCount`, `UserLastFireWoodTime` | `sc/user/UI/CUIArmyGroupCampsite.lua:2572` |
...
| **`OnGetAiBeiPayOrder`** | `RechargeLogic` | `orderId, strPostResult` | — | `sc/user/Logical/ClientRechargeLogic.lua:322` |
| **`OnGetAllCampStep`** | `CUIJFZYMain` | `tData` | — | `sc/user/UI/JFZY/CUIJFZYMain.lua:483` |
| **`OnGetAllCompleteChapterRank`** | `CUIJFZYMain` | `tData` | — | `sc/user/UI/JFZY/CUIJFZYMain.lua:456` |
...
| **`OnGetCampDamageRank`** | `CUIJFZYHurtRankDlg` | `data` | — | `sc/user/UI/JFZY/CUIJFZYHurtRankDlg.lua:235` |
...
| **`OnGetChapterStepInfo`** | `CUIJFZYChapterInfoDlg` | `data` | — | `sc/user/UI/JFZY/CUIJFZYChapterInfoDlg.lua:344` |
...
| **`OnGetCompleteAward`** | `CUIJFZYChapterInfoDlg` | `data` | — | `sc/user/UI/JFZY/CUIJFZYChapterInfoDlg.lua:418` |
...
| **`OnGetMVPInfo`** | `CUIJFZYMain` | `tData` | — | `sc/user/UI/JFZY/CUIJFZYMain.lua:463` |
| **`OnGetMailList`** | *(toàn cục)* | `tData` | `data`, `err`, `fun` | `sc/user/Logical/CServerResponseLogic.lua:921` |
| **`OnGetMovingHistory`** | `StateWarLogic` | `tData` | — | `sc/user/UI/StateWar/StateWarLogic.lua:74` |
...
| **`OnGetPaymentConfig`** | `CUIRechargeStore` | `tData` | `RetData`, `nClientErrCode` | `sc/user/UI/CUIRechargeStore.lua:137` |
...
| **`OnGetRandomName`** | `CUILogin` | `szName` | — | `sc/user/UI/CUILogin2.lua:2355` |
...
| **`OnGetShopConf`** | `CUIExchangeShop` | `data` | `costPrizeData` | `sc/user/UI/CUIExchangeShop.lua:447` |
...
| **`OnGetStepAward`** | `CUIJFZYChapterInfoDlg` | `data` | — | `sc/user/UI/JFZY/CUIJFZYChapterInfoDlg.lua:407` |
...
| **`OnGetWaterTreeReward`** | `CUIArmyGroupCampsite` | `—` | — | `sc/user/UI/CUIArmyGroupCampsite.lua:2414` |
...
| **`OnGuildWarCompeleteFight`** | `CUIGame` | `tData` | — | `sc/user/Battle/CUIGame.lua:4581` |
...
| **`OnJoinCamp`** | `CUIWorldWar` | `tData` | — | `sc/user/UI/CUIWorldWar.lua:430` |
...
| **`OnOneKeyAward`** | `CUIJFZYMain` | `tData` | — | `sc/user/UI/JFZY/CUIJFZYMain.lua:500` |
...
| **`OnQueryAccount`** | `CUIStockMainDlg` | `data` | — | `sc/user/UI/Stock/CUIStockMainDlg.lua:508` |
| **`OnQueryActivityShoppingLimit`** | `ClientActivityShoppingLogic` | `tData` | — | `sc/user/Logical/ClientActivitiesLogic.lua:404` |
...
| **`OnQueryStock`** | `CUIStockMainDlg` | `data` | — | `sc/user/UI/Stock/CUIStockMainDlg.lua:497` |
| **`OnQueryStockAllToday`** | `CUIStockMainDlg` | `data` | — | `sc/user/UI/Stock/CUIStockMainDlg.lua:485` |
...
| **`OnSellStock`** | `CUIStockDealDlg` | `data` | — | `sc/user/UI/Stock/CUIStockDealDlg.lua:226` |
...
| **`OnSetCountryNotice`** | `StateWarBattleData` | `data` | — | `sc/user/UI/StateWar/StateWarBattleData.lua:348` |
...
| **`OnSetStateWarLineUp`** | `StateWarBattleData` | `—` | — | `sc/user/UI/StateWar/StateWarBattleData.lua:41` |
...
| **`OnUpdateHp`** | `FBJiuFaZhongYuan` | `data` | — | `sc/user/FB/FBJiuFaZhongYuan.lua:365` |
...
<details><summary><code>ErrorCode.Arenal</code> — 22 mã</summary>
...
<details><summary><code>ErrorCode.ChargeSign</code> — 3 mã</summary>
...
<details><summary><code>ErrorCode.StateWar</code> — 13 mã</summary>
...
| 6720 | `TargetCityUnreachable` |
| 6725 | `MedalsRankDataNoExists` |
| 6726 | `NotTheMedalsRankChampion` |

</details>

<details><summary><code>ErrorCode.Stock</code> — 7 mã</summary>
...
<details><summary><code>ErrorCode.JFZY</code> — 14 mã</summary>

| mã | tên |
|---:|---|
| 6901 | `NotInJFZY` |
| 6902 | `NotInBattleTime` |
| 6903 | `NotInTheBattle` |
| 6904 | `GetHpListError` |
| 6905 | `WrongNPCID` |
| 6906 | `BattleOver` |
| 6907 | `NotAllowedBattle` |
| 6908 | `HeroWrong` |
| 6909 | `CostChanllengeFail` |
| 6910 | `HaveFinished` |
| 6911 | `NotInGroup` |
| 6913 | `CanNotAward` |
| 6914 | `Awarded` |
...
| `reqName` | GAME_LOGIC | `CUIAssist._addRpcFunc` | `sc/user/UI/CUIAssist.lua:355` |

## 7. Luật chơi dùng chung với server (`sc/share/`)

139 file logic client và server dùng chung — đây là phần quy tắc nghiệp vụ server phải tái hiện. Cột *hằng số* đếm các hằng số cấu hình khai báo trong `ctor`.
...
| `sc/share/CommonLogic.lua` | 175 | 0 | ÒÔÏÂÊÇÐèÒª¿ç·þ¹²ÏíµÄÂß¼­·½·¨ |
| `sc/share/Container.lua` | 68 | 1 | [[ |
| `sc/share/DestinyLogic.lua` | 527 | 1 | [[ |
| `sc/share/EndlessChapterLogic.lua` | 557 | 7 | [[ |
| `sc/share/EventManager.lua` | 1246 | 0 | Created by IntelliJ IDEA. |
...
| `sc/share/FightLogic.lua` | 1042 | 0 | 获取全部英雄战斗属性 |
...
| `sc/share/HeroSkinLogic.lua` | 645 | 0 |  |
| `sc/share/JiuFaZhongyuan/share_JiuFaZhongyuan.lua` | 218 | 2 | [[分组列表 类似活动关卡]] |
| `sc/share/KDebug.lua` | 840 | 0 | Date:		2014-05-21 |
...
| `sc/share/Pet/share_PetLogic.lua` | 229 | 32 | 等级是否可以超过玩家等级？ |
...
| `sc/share/PrizeLogic.lua` | 233 | 0 |  |
| `sc/share/Protocol.lua` | 1016 | 0 | 服务器类型序号 |
...
| `sc/share/Setting.lua` | 58 | 0 | 开启活动 |
...
| `sc/share/StarSoul/share_StarSoulLogic.lua` | 243 | 19 |  |
...
| `sc/share/error.lua` | 938 | 0 | system |
| `sc/share/jsonfile.lua` | 53 | 0 | [[ |
| `sc/share/public.lua` | 792 | 0 | 日志等级枚举 |
...
| `sc/share/share_HeroLogic.lua` | 3788 | 3 | [[ |
...
| `sc/share/share_configManager.lua` | 5064 | 0 | 配置管理器 |
| `sc/share/share_dailyTask.lua` | 92 | 0 |  |
| `sc/share/share_gameLogic_require.lua` | 170 | 0 |  |
